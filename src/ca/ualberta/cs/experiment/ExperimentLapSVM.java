...
public class ExperimentLapSVM
{
			...
			String delimiter) throws Exception
	{
		...
		{
			System.err.println("[LapSVM]Error while reading MST for dataset " + fileName);
			System.exit(-1);
		}
		...
		while (line != null) 
		{
			...
			for(Integer minPts: arrayMinPts)
			{
				...
				ExpLapSVM.propagateLabels(datasetExperiments, mKNN.get(minPts), preLabeledDenBased);

				fm    = fMeasureStatistic.getIndex(datasetExperiments.getObjects());
				report.get(parameters.FMClassification).get(parameters.lapSVM).get(nmc).get(plab).get(minPts).add(fm);
			...
			String stringFolder 		    = outputFolder + fileName + ".reportLapSvm";
			...
			{
				System.out.println("[LapSVM]An error ocurred while writing the report for data set " + fileName);
				e.printStackTrace();
				System.exit(-1);
			}

			System.out.println("[LapSVM]Dataset:  " + fileName + ". plab: " + plab + ". Trial: " + trial + ". time(min): " + (((System.currentTimeMillis() - timeTrial)/1000)/60));

		}// End for while

		reader.close();

		System.out.println("[LapSVM]Dataset:  " + fileName + ". Overall time(min): " + (((System.currentTimeMillis() - overallStartTime)/1000)/60));
