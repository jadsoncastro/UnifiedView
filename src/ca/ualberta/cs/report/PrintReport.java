...
public class PrintReport
{
	...
	public static void main(String args[]) throws IOException
	{
		...
		String tableStatistics = "Dataset\tAlgorithm\tNumber of Missed Classes\tPerc of labeled\tMinPts\tTrial\tPerformance\n";


		System.out.println("----Writing head of the table results " + "----");

		String tmp = stringFolder + "statisticalAnalysis-" + validationMethod + "-" + descriptionComparison + ".csv";
		...
		for(File fileEntry: folder.listFiles())
		{
			...
			if(!fileEntry.isHidden() && !fileEntry.isDirectory())
			{
				...
				if(dataFile.endsWith(".report"))
				{
					try(FileInputStream fileIn = new FileInputStream(stringFolder + dataFile);
							ObjectInputStream in   = new ObjectInputStream(fileIn);)
					{
						Map<String, Map<String, Map<Integer, Map<Integer, Map<Integer, ArrayList<Double>>>>>> reportForDataset = 
								(Map<String, Map<String, Map<Integer, Map<Integer, Map<Integer, ArrayList<Double>>>>>>) in.readObject();
	...
	private static String printTableStatistics(String stringData, Map<String, Map<Integer, Map<Integer, Map<Integer, ArrayList<Double>>>>> report, ArrayList<String> algorithmsList)
	{
		String response = "";

		for(Map.Entry<String, Map<Integer, Map<Integer, Map<Integer, ArrayList<Double>>>>> firstEntry: report.entrySet())
		{
			...
			for(Map.Entry<Integer, Map<Integer, Map<Integer, ArrayList<Double>>>> secondEntry: firstEntry.getValue().entrySet())
			{
				Integer nmc = secondEntry.getKey();
				for(Map.Entry<Integer, Map<Integer, ArrayList<Double>>> thirdEntry: secondEntry.getValue().entrySet())
				{
					Integer plab= thirdEntry.getKey();
					for(Map.Entry<Integer, ArrayList<Double>> fourthEntry: thirdEntry.getValue().entrySet())
					{
						Integer mPts = fourthEntry.getKey();
						ArrayList<Double> arrayResults = fourthEntry.getValue();

						for(int trial = 0; trial < arrayResults.size(); trial++)
						{
							response += stringData.toUpperCase() + "\t" + algorithm + "\t" + nmc + "\t" + plab + "\t" + mPts + "\t" + trial + "\t" + arrayResults.get(trial) + "\n";
						}
