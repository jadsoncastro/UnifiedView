...
public class RuntimeAllPointsCoreDistance
{
					...
					throws Exception
	{
		...
		try(FileInputStream fileIn = new FileInputStream(outputFolder + fileName + ".timeAllPointsCoreDistance");
				ObjectInputStream in = new ObjectInputStream(fileIn);)
		{
		...
		{
			System.err.println("[AP] Error while reading time results for dataset " + fileName);
			System.exit(-1);
		}
		...
		while (line != null) 
		{
			...
			for(String alg: algorithmsDesc)
			{
				...
				// Update ARI information time to propagate the labels and extract the clusters
			...
			for(Integer minPts: arrayMinPts)
			{
				if(idLabeled > -1)
				{
					...
					RuntimeHDBSCANVariations.BCubedBased(clustersStore.get(minPts), mstStore.get(minPts), matrixStore.get(minPts), preLabeledDenBased, datasetExperiments, minPts, report, timeStore, parameters.hdbscanStarBCubedAp, pmc, plab);

					Util.resetDataset(datasetExperiments, false);
					RuntimeHDBSCANVariations.MixedForConstraints(clustersStore.get(minPts), matrixStore.get(minPts), preLabeledDenBased, datasetExperiments, minPts, report, timeStore, parameters.hdbscanConstraintsAp, pmc, plab);
				}

				Util.resetDataset(datasetExperiments, false);
				RuntimeHDBSCANVariations.MixedBCubed(clustersStore.get(minPts), mstStore.get(minPts), matrixStore.get(minPts), preLabeledDenBased, datasetExperiments, minPts, report, timeStore, parameters.hdbscanStarMixedAp, pmc, plab);

				Util.resetDataset(datasetExperiments, false);
				RuntimeHDBSCANVariations.MixedForConstraints(clustersStore.get(minPts), matrixStore.get(minPts), preLabeledDenBased, datasetExperiments, minPts, report, timeStore, parameters.hdbscanStarMixedForConstraintsAp, pmc, plab);

				Util.resetDataset(datasetUnsupervised, true);
				RuntimeHDBSCANVariations.HdbscanUnsupervisedAndConstraints(clustersStore.get(minPts), matrixStore.get(minPts), null, datasetUnsupervised, minPts, report, timeStore, parameters.hdbscanStarAp, pmc, plab);
