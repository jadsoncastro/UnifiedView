...
public class ExperimentGFHF
{
			...
			DistanceCalculator distance, String delimiter) throws Exception
	{
		...
		{
			System.err.println("[GFHF]Error while reading Graph for dataset " + fileName);
			System.exit(-1);
		}
		...
		while (line != null) 
		{
			...
			for(Integer minPts: arrayMinPts)
			{
				...
				double timeHarmonic = ExpGRF.propagateLabels(datasetExperiments, mKNN.get(minPts), preLabeledDenBased);
				
				fm 		 = fMeasureStatistic.getIndex(datasetExperiments.getObjects());
				report.get(parameters.FMClassification).get(parameters.harMKNN).get(nmc).get(plab).get(minPts).add(fm);
			}
			...
			String stringFolder 		    = outputFolder + fileName + ".reportGfhf";
