...
import static ca.ualberta.cs.hdbscanApts.HDBSCANApts.WARNING_MESSAGE;
 ...
 * @author jadson
 */
public class HDBSCANAptsRunner {
	...
	private static final String TANIMOTO_SIMILARITY   = "tanimoto";
	...
	public static void main(String[] args) 
	{
		...
		HDBSCANAptsParameters parameters = checkInputParameters(args, HMatrix);
		...
		System.out.println("Running HDBSCAN*(APTS) on " + parameters.inputFile + ", minClSize=" + parameters.minClusterSize + 
				", constraints=" + parameters.constraintsFile + ", compact=" + parameters.compactHierarchy + 
				", dist_function=" + parameters.distanceFunction.getName() + ", outputExtension="+ parameters.outType);
		
		//Read in input file:
		double[][] dataSet = null;
		try {
			dataSet = HDBSCANApts.readInDataSet(parameters.inputFile, ",");		
		}
		...
		if (parameters.constraintsFile != null) {
			try {
				constraints = HDBSCANApts.readInConstraints(parameters.constraintsFile, ",");
			}
			catch (IOException e) {
		...
		double[] coreDistances = HDBSCANApts.calculateCoreDistances(dataSet, parameters.distanceFunction);
		System.out.println("Time to compute core distances (ms): " + (System.currentTimeMillis() - startTime));
		System.out.println("Core distances" + Arrays.toString(coreDistances));

		//Calculate minimum spanning tree:
		startTime = System.currentTimeMillis();
		UndirectedGraph mst = HDBSCANApts.constructMST(dataSet, coreDistances, true, parameters.distanceFunction);
		...
		HDBSCANStarRunner.WrapInt lineCount = new HDBSCANStarRunner.WrapInt(0);
		ArrayList<Cluster> clusters = null;
		try {
			startTime = System.currentTimeMillis();
			
			clusters = HDBSCANApts.computeHierarchyAndClusterTree(mst, parameters.minClusterSize,
		...
		//Propagate clusters:
		boolean infiniteStability = HDBSCANApts.propagateTree(clusters, "unsupervised");
                ...
                {                    
                    ...
                    try {
                            startTime = System.currentTimeMillis();
                            int[] flatPartitioningSHM = HDBSCANApts.findProminentClustersSHM(clusters, HMatrix);
                ...
                else //Comment if you wish to compare the time
                {
                    ...
                    try {
                            ...
                            int[] flatPartitioning = HDBSCANApts.findProminentClusters(clusters, parameters.hierarchyFile, parameters.partitionFile, 
		...
		try {
			startTime = System.currentTimeMillis();
			HDBSCANApts.calculateOutlierScores(clusters, pointNoiseLevels, pointLastClusters, 
		...
		//Generating .vis file
		if(!parameters.outType.equals(HDBSCANAptsRunner.SHM_OUT))
		{
			String out = "";
	 ...
	 */
	private static HDBSCANAptsParameters checkInputParameters(String[] args, SHM.HMatrix.HMatrix HMatrix) {
		HDBSCANAptsParameters parameters = new HDBSCANAptsParameters();
		...
		for (String argument : args) {
			...
			//Assign distance function:
				...
				else if (functionName.equals(TANIMOTO_SIMILARITY))
				{
					parameters.distanceFunction = new TanimotoSimilarity();
					HMatrix.setParam_distanceFunction(TANIMOTO_SIMILARITY);
				}
	...
	/**
	 * Prints a help message that explains the usage of HDBSCANAptsRunner, and then exits the program.
	 */
	private static void printHelpMessageAndExit() {
		System.out.println();
		
		System.out.println("Executes the HDBSCAN*(APTS) algorithm, which produces a hierarchy, cluster tree, " +
				"flat partitioning, and outlier scores for an input data set.");
		System.out.println("Usage: java -jar HDBSCANApts.jar file=<input file> " + 
		...
		System.out.println("Example usage: \"java -jar HDBSCANApts.jar file=input.csv minClSize=4\"");
		System.out.println("Example usage: \"java -jar HDBSCANApts.jar file=collection.csv minClSize=1 " + 
				"constraints=collection_constraints.csv dist_function=manhattan\"");
		System.out.println("Example usage: \"java -jar HDBSCANApts.jar file=data_set.csv minClSize=8 " + 
				"compact=true\"");
		System.out.println("In cases where the source is compiled, use the following: \"java HDBSCANAptsRunner " +
		...
		System.out.println("tanimoto: Tanimoto Similarity, d = 1 - (sum(x1*y1, ..., xn*yn)/(sum(x1*x1, ..., xn*xn) + sum(y1*y1, ..., yn*yn) -sum(x1*y1, ..., xn*yn)))");
		System.out.println();
	 ...
	 */
	private static class HDBSCANAptsParameters {
